...
pub fn search(findings: &[SqlCase], pool: &Pool) -> SearchResult {
...
#[cfg(test)]
mod tests {
    /// `n choose k`, for computing the expected enumeration size from the vocabulary.
    fn combinations(n: usize, k: usize) -> usize {
        (0..k).fold(1, |total, i| total * (n - i) / (i + 1))
    }
    ...
    /// The case the search's tests key on: one carrying a `NULL`.
    fn overflowing() -> SqlCase {
        case_with_null()
    }
    ...
    fn pool(cases: Vec<(SqlCase, Source)>) -> Pool {
        Pool::unchecked(
            cases
                .into_iter()
    ...
    fn the_enumeration_covers_every_signed_combination_of_at_most_three_features() {
        ...
        // **Computed, not hardcoded.** The tensor version asserted 6,018 — the count for its
        // seventeen features. That number is a property of the *vocabulary*, not of the
        // enumeration, so carrying it across domains would have made a correct enumeration
        // look broken. For n features the count of signed conjunctions of at most three is
        // `sum over k in 1..=3 of C(n,k) * 2^k`, which is 9,920 for twenty.
        let expected: usize = (1..=MAX_FEATURES)
            .map(|k| combinations(FEATURES.len(), k) * 2usize.pow(k as u32))
            .sum();
    ...
    fn the_covering_loop_terminates() {
        ...
        let expected: usize = (1..=MAX_FEATURES)
            .map(|k| combinations(FEATURES.len(), k) * 2usize.pow(k as u32))
            .sum();
        assert_eq!(result.considered, expected);
    }
