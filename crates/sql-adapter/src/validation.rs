...
///
/// Rust note: `impl FnMut(&SqlCase) -> bool` takes any closure that may hold mutable state
/// (a backend handle, a counter). `impl Generator<In = SqlCase>` is the same idea for the
/// source of cases: this function names the capability it needs, not a concrete type.
pub fn validate(
    predicate: Predicate,
    generator: &impl Generator<In = SqlCase>,
    seed: u64,
    budget: usize,
    mut diverges: impl FnMut(&SqlCase) -> bool,
) -> Validation {
...
mod tests {
    ...
    fn generator() -> SqlGenerator {
        SqlGenerator::new(Bounds::V1_ALL)
    }
    ...
    fn a_rule_whose_matched_cases_always_diverge_is_a_trigger() {
        let everything_diverges = |_: &SqlCase| true;
    ...
    fn a_rule_whose_matched_cases_never_diverge_is_a_coincidence() {
        let nothing_diverges = |_: &SqlCase| false;
        let predicate = Predicate::new(&[], &["outer_join_present"]);
    ...
    fn a_rule_the_generator_never_produces_is_reported_as_unreachable_not_as_failed() {
        ...
        // The tensor version used a feature its generator structurally could not produce.
        // **No single SQL feature is unreachable** — `features.rs` asserts every one occurs.
        // So the unreachable rule here exploits an *implication* inside the vocabulary:
        // `set_op_deduplicates` is only ever set alongside `set_op_present`, so requiring the
        // first while forbidding the second is unsatisfiable by construction.
        //
        // The first attempt at this used "empty table AND duplicate rows", which looks
        // contradictory and is not: both features are computed across *all* tables, so one
        // table can be empty while another has duplicates. Worth recording — a rule that
        // reads as impossible may simply be describing two different tables.
        let unreachable = Predicate::new(&["set_op_deduplicates"], &["set_op_present"]);
