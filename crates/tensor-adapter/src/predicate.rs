...
//! some that must be clear. Rendered as `mixed_sign_overflow ∧ ¬rank_ge_3`.
//!
...
impl Predicate {
    ...
    /// Render as the rule a person reads: `mixed_sign_overflow ∧ ¬rank_ge_3`.
...
mod tests {
    ...
    /// The burn#5284 case, whose features are known and measured.
    fn filed_case() -> TensorOp {
    ...
    fn a_predicate_built_from_names_still_matches_the_case_that_produces_it() {
        let predicate = Predicate::new(&["overflow_product", "mixed_sign_overflow"], &[]);
    ...
    fn a_required_feature_that_is_absent_fails_the_match() {
        ...
        // match it.
        let predicate = Predicate::new(&["overflow_product", "rank_ge_3"], &[]);
    ...
    fn a_rule_can_match_a_case_that_does_not_diverge() {
        ...
        let agreeing = matmul((&[1, 4], &ones), (&[4, 1], &ones));

        let too_broad = Predicate::new(&["all_same_sign"], &[]);
    ...
    fn size_counts_both_required_and_forbidden() {
        let predicate = Predicate::new(&["overflow_product"], &["rank_ge_3", "k_large"]);
    ...
    fn a_rule_renders_readably_with_negation_marked() {
        let rendered = Predicate::new(&["mixed_sign_overflow"], &["rank_ge_3"]).describe();

        assert!(rendered.contains("mixed_sign_overflow"));
        assert!(rendered.contains("¬rank_ge_3"));
    ...
    fn an_unknown_feature_name_is_ignored_rather_than_fatal() {
        let predicate = Predicate::new(&["overflow_product", "no_such_feature"], &[]);
